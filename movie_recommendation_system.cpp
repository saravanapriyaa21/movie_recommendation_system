...
unordered_map<int, vector<double>> shuffleRatings(
    ...
    for (auto &p : partials)
        for (auto &e : p)
            merged[e.first].insert(merged[e.first].end(),
...
vector<int> recommendCollaborative(int uid, const unordered_map<int, set<int>>& wh) {
    if (!wh.count(uid) || wh.at(uid).empty()) return {};
...
void buildUserHistory(int uid) {
    ...
    auto movies = readCSV("data/processed/movies_processed.csv");
    auto watch  = readCSV("data/processed/watch_history.csv");

    for (auto &m : movies) {
        if (m.size() < 4 || !isNumber(m[0])) continue;
        double rating = 0.0;
    ...
    for (auto &w : watch) {
        if (w.size() < 3 || !isNumber(w[0]) || !isNumber(w[1])) continue;
        if (stoi(w[0]) == uid)
...
string mostWatchedGenre() {
    if (user_watch_history.empty()) return "";
...
void surpriseMe(int uid,
    ...
    vector<int> pool;
    for (auto &m : movies)
        if (!wh.count(uid) || !wh.at(uid).count(m.first))
...
int main() {
    ...
    auto watchMap = loadWatchHistoryMap("data/processed/watch_history.csv");
    ...
    while (true) {
        ...
        if (ch == 1) {
            ...
            if (g.empty()) {
                cout << "\nNo watch history found.\n"
                     << "Watch more movies to get genre-based recommendations!\n";
            } else {
                cout << "\nFAVOURITE GENRE: " << g << "\n\n";
